...
class ProxyManager:
    ...
    def DownloadFromConfig(cf, Proxy_type: int) -> Set[Proxy]:
        ...
        logger.info(
            f"{Colors.WARNING}Downloading Proxies from {Colors.OKBLUE}%d{Colors.WARNING} Providers{Colors.RESET}" % len(
                providrs))
    ...
    def download(provider, proxy_type: ProxyType) -> Set[Proxy]:
        logger.debug(
            f"{Colors.WARNING}Proxies from (URL: {Colors.OKBLUE}%s{Colors.WARNING}, Type: {Colors.OKBLUE}%s{Colors.WARNING}, Timeout: {Colors.OKBLUE}%d{Colors.WARNING}){Colors.RESET}" %
